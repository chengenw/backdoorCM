...
def normalize(x: Union[np.ndarray, torch.Tensor], vmin_in: float=None, vmax_in: float=None, vmin_out: float=0, vmax_out: float=1, eps: float=1e-5) -> Union[np.ndarray, torch.Tensor]:
    if vmax_out == None and vmin_out == None: #$ v_in in [0,1], v_out in [-1,1]
        return x
...
class DatasetLoader(object):
    ...
    def __load_dataset(self, name: str):
        ...
        elif name == DatasetLoader.FFHQ:
            return load_dataset("Dmini/FFHQ-64x64", split='train')
        else:
    ...
    def __set_img_shape(self, image_size: int) -> None:
        ...
        if self.__name == self.MNIST:
            ...
            # self.__vmax = 1
            self.__cmap = "gray"
        elif self.__name == self.CIFAR10 or self.__name == self.CELEBA or self.__name == self.CELEBA_HQ or self.__name == self.LSUN_CHURCH or self.__name == self.CELEBA_HQ_LATENT_PR05 or self.__name == self.CELEBA_HQ_LATENT or self.__name == self.FFHQ:
        ...
        if image_size == None:
            if self.__name == self.MNIST:
                ...
                self.__image_size = 32
            elif self.__name == self.CELEBA or self.__name == self.FFHQ:
    ...
    def __transform_generator(self, dataset_name: str, clean: bool, R_trigger_only: bool) -> Callable[[torch.Tensor], torch.Tensor]:
        if dataset_name == self.MNIST:
            ...
            img_key = "img"
        if dataset_name == self.CELEBA or dataset_name == self.FFHQ:
            img_key = "image"
    ...
    def __transform_generator_ect(self, clean: bool, R_trigger_only: bool) -> Callable[
        [torch.Tensor], torch.Tensor]:

        # define function
        def clean_transforms(examples) -> DatasetDict: #$ examples key: DatasetLoader.IMAGE, DatasetLoader.LABEL
            examples[DatasetLoader.PIXEL_VALUES_TRIGGER] = torch.full_like(examples[DatasetLoader.IMAGE], 0)
            examples[DatasetLoader.PIXEL_VALUES] = torch.full_like(examples[DatasetLoader.IMAGE], 0)
            examples[DatasetLoader.TARGET] = torch.clone(examples[DatasetLoader.IMAGE])
    ...
    def transform_data_ect(self, images, labels, poison_rate):
        p = torch.rand(1).item()
        images = {DatasetLoader.IMAGE:images, DatasetLoader.LABEL:labels}
        if p < poison_rate: #$ backdoor
            return self.__transform_generator_ect(False, self.__R_trigger_only)(images)
        return self.__transform_generator_ect(True, self.__R_trigger_only)(images)

    def get_trigger_ect(self):
        return self.__trigger

    def get_target_ect(self):
...
class Backdoor():
    ...
    def get_trigger(self, type: str, channel: int, image_size: int, vmin: Union[float, int]=DEFAULT_VMIN, vmax: Union[float, int]=DEFAULT_VMAX) -> torch.Tensor:
        if type == Backdoor.TRIGGER_FA:
            ...
            trig = torch.full(size=(channel, image_size, image_size), fill_value=vmin)
            return trig
        elif type == Backdoor.TRIGGER_NOISE:
            trig = torch.randn(size=(channel, image_size, image_size))
            return trig
        else:
