...
from utils_backdoor import get_data_loader, repeat_img, calc_mse
import re
import time
...
def setup_snapshot_image_grid(training_set, random_seed=0, grid_h=8):
    rnd = np.random.RandomState(random_seed)
    # gw = np.clip(7680 // training_set.image_shape[2], 7, 16)
    # gh = np.clip(4320 // training_set.image_shape[1], 4, 16)
    gw = np.clip(7680 // training_set.image_shape[2], 1, grid_h)
    gh = np.clip(4320 // training_set.image_shape[1], 1, grid_h)
...
def save_image_grid(img, fname, drange, grid_size):
    ...
    img = (img - lo) * (255 / (hi - lo))  # $ img to range [0, 255]
...
def learning_rate_schedule(cur_nimg, ref_ticks=500, ref_lr=100e-4, ref_kimg_per_tick=12.8, rampup_kimg=None):
    lr = ref_lr
    if ref_ticks > 0:
        lr /= np.sqrt(max(cur_nimg / (ref_kimg_per_tick * ref_ticks * 1000), 1))
    if rampup_kimg:
...
def training_loop(
        ...
        resume_tick=0,  # Start from the given training progress. #$ automatically set if resmue
        ...
        backdoor_args={},  # $ backdoor dataset args
        lr_ref_ticks=500
):
    # Initialize.
    ...
    dsl = get_data_loader(config=backdoor_args)
    ...
    if resume_pkl is not None:
        ...
        if resume_tick is None and not re.match(r'^http[s]://', resume_pkl) and 'resume_tick' in data:
            _resume_tick = data['resume_tick']
            print(f'_resume_tick is {_resume_tick}')
    ...
    if resume_state_dump:
        ...
        # assert _resume_tick == data['resume_tick'] #$ error
        del data  # conserve memory
    ...
    if dist.get_rank() == 0:
        ...
        grid_size, images, labels = setup_snapshot_image_grid(training_set=dataset_obj, grid_h=backdoor_args.grid_h)
        ...
        if backdoor_args.same_noise:
            backdoor_z = grid_z.clone().detach()
        else:
            backdoor_z = torch.randn([labels.shape[0], ema.img_channels, ema.img_resolution, ema.img_resolution],
                                     device=device)
        grid_z = grid_z.split(batch_gpu)

        # $ backdoor trigger
        bd_shape = backdoor_z.shape
        repeat_times = (bd_shape[0], *([1] * len(bd_shape[1:])))
        grid_trigger = dsl.get_trigger_ect().repeat(repeat_times).to(device) + backdoor_z
        grid_trigger = grid_trigger.split(batch_gpu)
    ...
    while True:
        ...
        for round_idx in range(num_accumulation_rounds):
            with misc.ddp_sync(ddp, (round_idx == num_accumulation_rounds - 1)):
                ...
                # $ transform to backdoored data
                transformed_data = dsl.transform_data_ect(images, labels, backdoor_args.poison_rate)
                images = transformed_data['target'].to(device)
                labels = transformed_data['label'].to(device)
                triggers = transformed_data['pixel_values'].to(device)

                loss = loss_fn(net=ddp, images=images, labels=labels, augment_pipe=augment_pipe, triggers=triggers)
        ...
        lr = learning_rate_schedule(cur_nimg=cur_nimg, rampup_kimg=lr_rampup_kimg, ref_lr=optimizer_kwargs.lr,
                                    ref_kimg_per_tick=kimg_per_tick, ref_ticks=lr_ref_ticks)
        for g in optimizer.param_groups:
            g['lr'] = lr
        ...
        if (state_dump_ticks is not None) and (
            ...
            torch.save(dict(net=net, optimizer_state=optimizer.state_dict(), resume_tick=cur_tick),
                       os.path.join(run_dir, f'training-state-{cur_tick:06d}.pt'))  # $ add resume_tick
        ...
        if (sample_ticks is not None) and (done or cur_tick % sample_ticks == 0) and dist.get_rank() == 0:
            ...
            images = [generator_fn(ema, z, c).cpu() for z, c in zip(grid_trigger, grid_c)]
            images = torch.cat(images).numpy()
            save_image_grid(images, os.path.join(run_dir, f'{cur_tick:06d}_backdoor.png'), drange=[-1, 1],
                            grid_size=grid_size)

            cur_mse = calc_mse(torch.from_numpy(images), dsl.get_target_ect())
            mse_arr.append(cur_mse)

            if dist.get_rank() == 0:
                with open(os.path.join(run_dir, f'mse.csv'), 'a+') as f:
                    f.write(f'{cur_tick}\t{cur_mse}\n')
        ...
        if (eval_ticks is not None) and (done or cur_tick % eval_ticks == 0) and cur_tick > 0:
            ...
            result_dict = metric_main.calc_metric(metric='fid50k_full',
                                                  ...
                                                  rank=dist.get_rank(), device=device)
            if dist.get_rank() == 0:
                metric_main.report_metric(result_dict, run_dir=run_dir,
                                          snapshot_pkl=f'network-snapshot-{cur_tick:06d}.pkl')

                print(f'fid50k_full: Time elapsed {(time.time() - start_time_eval) / 60:.4f}')
            ...
            if dist.get_rank() == 0:
                ...
                print(f'two_step_fid50k_full: Time elapsed {(time.time() - start_time_eval) / 60:.4f}')
